...
const SupplierForm = ({
  ...
  const handleSubmit = async (e) => {
    if (mode === "add") {
      ...
      const { success, error } = await clientFetch(adminSupplierApi.create(newSupplier))
      if (!success) {
        setVerifyInput((prev) => ({
          ...prev,
          common: error,
      ...
      const updatedSupplier = { name: currentSupplierClicked.name }
      const updatedId = currentSupplierClicked.id
      const { success, error } = await clientFetch(adminSupplierApi.update(updatedId, updatedSupplier))
      if (!success) {
        setVerifyInput((prev) => ({
          ...prev,
          common: error,
  ...
  const handleDelete = async (e) => {
    ...
    if (isSure == "xóa") {
      const { success, error } = await clientFetch(adminSupplierApi.delete(currentSupplierClicked.id))
      if (!success) {
        setVerifyInput((prev) => ({
          ...prev,
          common: error,
